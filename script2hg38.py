...
os.system("bedtools intersect -a bed_chr_{}_sorted.bed -b {} > resultgene{}.bed".format(sys.argv[3],sys.argv[1],sys.argv[3]))
...
w3 = open("gccoords_{}2def.bed".format(sys.argv[3]),"w")
a  = ""
for line in op:
        line = line.strip()
        line = line.split()
...
os.system("uniq gccoords_{}2def.bed > gccoords_{}_undef.bed".format(sys.argv[3],sys.argv[3]))
os.system("uniq resultgene{}.bed | awk  '!seen[$4]++' > resultgene{}_un.bed".format(sys.argv[3],sys.argv[3]))
os.system("bedtools intersect  -a bed_chr_{}_sorted.bed -b gccoords_{}_undef.bed | awk  '!seen[$4]++' > intmut{}.bed".format(sys.argv[3],sys.argv[3],sys.argv[3])) 
d1 = subprocess.check_output('wc -l intmut{}.bed'.format(sys.argv[3]),shell = True) #look at the number of mutations
d2 = subprocess.check_output('wc -l resultgene{}_un.bed'.format(sys.argv[3]), shell = True)
