...
def find_matching_sequence(fasta_file, target_length, target_gc,w):
...
for d in range(1,25,1):
    ...
    w = open("non_quadr_gc_nonok2_rep{}.txt".format(d),"a")
    for line in op:
        if "#Sequence" not in line:
            ...
            w.flush()
            result = find_matching_sequence(handle, length, gc, w)
    w.close()
    op.close()
